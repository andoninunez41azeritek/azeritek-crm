...
const AV_COLORS = { AN: '#8b7fff', JO: '#1fc98a', IK: '#3b9eff', MI: '#f5a623' }

const DEFAULT_TYPES = [
  { name: 'Industrial', color: '#a78bfa' },
  { name: 'Tecnología', color: '#3b9eff' },
  { name: 'Construcción', color: '#f5a623' },
  { name: 'Retail', color: '#1fc98a' },
  { name: 'Servicios', color: '#60a5fa' },
  { name: 'Clínicas', color: '#ef4444' },
  { name: 'Hostelería', color: '#fb923c' },
  { name: 'Automoción', color: '#ec4899' },
  { name: 'Logística', color: '#fbbf24' },
  { name: 'Inmobiliaria', color: '#34d399' },
  { name: 'Educación', color: '#c084fc' },
  { name: 'Alimentación', color: '#8b7fff' },
]
...
export default function Home() {
  ...
  const [newCo, setNewCo] = useState({ name: '', contact: '', email: '', phone: '', type: '', service: 'Agente IA WhatsApp' })
  ...
  async function loadData() {
    ...
    if (c.data?.length) setCompanies(c.data)
    if (l.data?.length) setLeads(l.data)
    if (t.data?.length) setTypes(t.data)
    if (a.data?.length) setAppointments(a.data)
    setLoading(false)
  }
  ...
  async function addCompany() {
    ...
    setNewCo({ name: '', contact: '', email: '', phone: '', type: '', service: 'Agente IA WhatsApp' })
    setShowAddCo(false)
  }
  ...
  async function deleteType(idx) {
    const t = types[idx]
    await supabase.from('types').delete().eq('name', t.name)
  ...
  function getTypeColor(name) {
    return types.find(t => t.name === name)?.color || '#7880a0'
  }
  ...
  appointments.forEach(a => {
    ...
    CAL_EVENTS[d].push(a.company + ' · ' + a.time?.slice(0,5))
  })
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', background: '#0b0d12', color: '#edf0f8', fontFamily: 'var(--font-sans)' }}>
      <Sidebar page={page} setPage={setPage} role={role} onLogout={() => setRole(null)} leads={leads} />
      <div style={{ flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column' }}>
        <Topbar title={{ dashboard:'Dashboard', calendar:'Calendario', companies:'Empresas', pipeline:'Pipeline', types:'Tipos de empresa', team:'Equipo', reports:'Informes' }[page]} role={role} onNewCita={() => setPage('calendar')} onAddCo={() => { setPage('companies'); setShowAddCo(true) }} />
        <div style={{ padding: '20px 24px', flex: 1 }}>
          {loading && <div style={{ color: '#7880a0', fontSize: 13 }}>Cargando...</div>}

          {page === 'dashboard' && <Dashboard leads={leads} companies={companies} setPage={setPage} />}

          {page === 'calendar' && (
            <CalendarPage
              events={CAL_EVENTS} showForm={showCitaForm} setShowForm={setShowCitaForm}
              companies={companies} onSave={async (cita) => {
                const { data } = await supabase.from('appointments').insert([cita]).select()
          ...
          {page === 'companies' && (
            <CompaniesPage
              companies={filteredCos} types={types} filter={coFilter} setFilter={setCoFilter}
              allCompanies={companies} showAdd={showAddCo} setShowAdd={setShowAddCo}
              newCo={newCo} setNewCo={setNewCo} onSave={addCompany}
            />
          )}

          {page === 'pipeline' && (
            <PipelinePage
              leads={filteredLeads} allLeads={leads} filter={pipeFilter} setFilter={setPipeFilter}
              showNew={showNewLead} setShowNew={setShowNewLead}
              newLead={newLead} setNewLead={setNewLead} onSave={addLead}
            />
          )}

          {page === 'types' && role === 'admin' && (
            <TypesPage types={types} companies={companies} newType={newType} setNewType={setNewType}
              selColor={selColor} setSelColor={setSelColor} onAdd={addType} onDelete={deleteType} colors={COLORS} />
          )}
...
function LoginScreen({ onLogin }) {
  ...
  const [user, setUser] = useState('andoni@azeritek.es')
    ...
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'#0b0d12' }}>
      <div style={{ background:'#13161f', border:'1px solid rgba(255,255,255,.2)', borderRadius:18, padding:36, width:360 }}>
        <div style={{ display:'flex', gap:12, marginBottom:24, alignItems:'center' }}>
          <div style={{ width:40, height:40, background:'#8b7fff', borderRadius:11, display:'flex', alignItems:'center', justifyContent:'center', fontWeight:700, fontSize:17, color:'#fff' }}>A</div>
          <div><div style={{ fontSize:17, fontWeight:700, color:'#edf0f8' }}>Azeritek</div><div style={{ fontSize:11, color:'#7880a0' }}>Panel de gestión comercial</div></div>
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:9, marginBottom:22 }}>
          {['admin','comercial'].map(role => (
            <div key={role} onClick={() => { setR(role); setUser(role==='admin'?'andoni@azeritek.es':'iker@azeritek.es') }}
              style={{ border:`1px solid ${r===role?'#8b7fff':'rgba(255,255,255,.2)'}`, borderRadius:11, padding:15, cursor:'pointer', textAlign:'center', background: r===role?'rgba(139,127,255,.12)':'#1c2030' }}>
              <div style={{ fontSize:22, marginBottom:6 }}>{role==='admin'?'🛡️':'💼'}</div>
              <div style={{ fontSize:12, fontWeight:600, color: r===role?'#edf0f8':'#b8c0d8' }}>{role==='admin'?'Administrador':'Comercial'}</div>
            </div>
          ))}
        </div>
        <FInput label="Usuario" value={user} onChange={setUser} />
        <FInput label="Contraseña" value={pass} onChange={setPass} type="password" />
        <Btn variant="primary" style={{ width:'100%', justifyContent:'center', padding:'11px 0', marginTop:4 }} onClick={() => onLogin(r)}>Acceder →</Btn>
...
// ── SIDEBAR ────────────────────────────────────────────
function Sidebar({ page, setPage, role, onLogout, leads }) {
  const items = [
    { id:'dashboard', icon:'🏠', label:'Dashboard' },
    { id:'calendar', icon:'📅', label:'Calendario', badge: 3 },
    ...
    <div style={{ width:220, background:'#13161f', borderRight:'1px solid rgba(255,255,255,.12)', display:'flex', flexDirection:'column', flexShrink:0 }}>
      ...
      <div style={{ padding:12, borderTop:'1px solid rgba(255,255,255,.12)' }}>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          <div style={{ width:34, height:34, borderRadius:'50%', background:'rgba(139,127,255,.25)', color:'#c4beff', display:'flex', alignItems:'center', justifyContent:'center', fontSize:12, fontWeight:700 }}>A</div>
          <div style={{ flex:1 }}>
            <div style={{ fontSize:13, fontWeight:600, color:'#edf0f8' }}>Andoni</div>
...
function Dashboard({ leads, companies, setPage }) {
  const mrr = leads.filter(l=>l.status==='mantenimiento').reduce((s,l)=>s+(l.monthly||0),0)
  return (
    <div>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12, marginBottom:20 }}>
        {[
          { icon:'🏢', val: companies.length, label:'Empresas en cartera', delta:'+3 este mes', color:'rgba(139,127,255,.15)', ic:'#c4beff' },
          { icon:'📊', val: leads.length, label:'Leads en pipeline', delta:'1 presupuesto pendiente', color:'rgba(245,166,35,.15)', ic:'#fcd34d' },
          { icon:'✅', val: leads.filter(l=>l.status==='pedido').length, label:'Pedidos confirmados', delta:'este trimestre', color:'rgba(31,201,138,.15)', ic:'#6ee7b7' },
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:16 }}>
        <Card title="Actividad reciente" sub="Últimas acciones">
        ...
        <Card title="Alertas" sub={<span style={{ color:'#ef4444' }}>pendientes</span>}>
          {leads.filter(l=>l.flag_color==='red').slice(0,4).map((l,i) => (
            <div key={i} style={{ background:'rgba(239,68,68,.1)', border:'1px solid rgba(239,68,68,.2)', borderRadius:10, padding:'10px 14px', marginBottom:8 }}>
              ...
              <div style={{ fontSize:12, color:'#b8c0d8', marginTop:2 }}>{l.sector} · fecha: {l.flag_date}</div>
            </div>
          ))}
          {leads.filter(l=>l.flag_color==='red').length===0 && <div style={{ fontSize:13, color:'#7880a0' }}>Sin alertas pendientes ✓</div>}
        </Card>
      </div>
      <Card title="Resumen del pipeline" action={<Btn onClick={()=>setPage('pipeline')}>Ver pipeline →</Btn>}>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(6,1fr)', gap:8 }}>
          {[{id:'contactado',label:'Contactado',color:'#3b9eff'},{id:'pendiente_presupuesto',label:'Pdte. presup.',color:'#f5a623'},{id:'presupuesto_enviado',label:'Presup. enviado',color:'#8b7fff'},{id:'pedido',label:'Pedido ✓',color:'#1fc98a'},{id:'mantenimiento',label:'Mantenim.',color:'#14b8a6'},{id:'perdido',label:'Perdido',color:'#ef4444'}].map(col => (
...
// ── CALENDAR ───────────────────────────────────────────
function CalendarPage({ events, showForm, setShowForm, companies, onSave }) {
  const [cita, setCita] = useState({ company:'', date:'', time:'10:00', service:'Agente IA WhatsApp', assigned:'Andoni', notes:'' })
  return (
    <div>
      <Card title="Junio 2025" sub="5 citas este mes" action={<Btn variant="primary" onClick={()=>setShowForm(!showForm)}>+ Nueva cita</Btn>}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap:4 }}>
          {Array.from({length:30},(_,i)=>i+1).map(d=>(
            <div key={d} style={{ minHeight:54, background: events[d]?'rgba(139,127,255,.1)':'#1c2030', border:`1px solid ${d===12?'rgba(139,127,255,.7)':events[d]?'rgba(139,127,255,.4)':'rgba(255,255,255,.12)'}`, borderRadius:9, padding:'6px 7px', cursor:'pointer' }}>
              <div style={{ fontSize:12, fontWeight:600, color: d===12?'#c4beff':'#b8c0d8', marginBottom:3 }}>{d}</div>
              {events[d]?.map((ev,i)=><div key={i} style={{ fontSize:9, background:'#8b7fff', color:'#fff', borderRadius:4, padding:'2px 5px', marginBottom:2, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{ev}</div>)}
            </div>
          ))}
        </div>
      </Card>
      {showForm && (
        <Card title="Nueva cita">
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:12 }}>
            <FInput label="Empresa" value={cita.company} onChange={v=>setCita({...cita,company:v})} />
            <FInput label="Fecha" type="date" value={cita.date} onChange={v=>setCita({...cita,date:v})} />
            <FInput label="Hora" type="time" value={cita.time} onChange={v=>setCita({...cita,time:v})} />
          </div>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            <div style={{ marginBottom:12 }}>
              <label style={{ fontSize:12, color:'#b8c0d8', marginBottom:5, display:'block', fontWeight:500 }}>Servicio</label>
              <select className="fi" value={cita.service} onChange={e=>setCita({...cita,service:e.target.value})} style={{ width:'100%', padding:'9px 12px', border:'1px solid rgba(255,255,255,.2)', borderRadius:9, fontSize:13, background:'#1c2030', color:'#edf0f8', outline:'none' }}>
                {['Agente IA WhatsApp','Reservas automáticas','CRM + integraciones','Web premium','Atención 24/7','Email automation'].map(s=><option key={s}>{s}</option>)}
              </select>
            </div>
            <FInput label="Comercial" value={cita.assigned} onChange={v=>setCita({...cita,assigned:v})} />
          </div>
          <FInput label="Notas" value={cita.notes} onChange={v=>setCita({...cita,notes:v})} />
          <div style={{ display:'flex', gap:8 }}>
            <Btn variant="primary" onClick={()=>onSave(cita)}>✓ Guardar cita</Btn>
...
// ── COMPANIES ──────────────────────────────────────────
function CompaniesPage({ companies, types, filter, setFilter, allCompanies, showAdd, setShowAdd, newCo, setNewCo, onSave }) {
  return (
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
        {companies.map((c,i) => {
          ...
          return (
            <div key={i} style={{ background:'#1a1f2e', border:'1px solid rgba(255,255,255,.18)', borderRadius:14, padding:15, cursor:'pointer' }}>
      ...
      {showAdd && (
        <Card title="Nueva empresa" style={{ marginTop:14 }}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            <div style={{ marginBottom:12 }}>
              <label style={{ fontSize:12, color:'#b8c0d8', marginBottom:5, display:'block', fontWeight:500 }}>Tipo</label>
              <select value={newCo.type} onChange={e=>setNewCo({...newCo,type:e.target.value})} style={{ width:'100%', padding:'9px 12px', border:'1px solid rgba(255,255,255,.2)', borderRadius:9, fontSize:13, background:'#1c2030', color:'#edf0f8', outline:'none' }}>
                {types.map(t=><option key={t.name} value={t.name}>{t.name}</option>)}
              </select>
            </div>
            <div style={{ marginBottom:12 }}>
              <label style={{ fontSize:12, color:'#b8c0d8', marginBottom:5, display:'block', fontWeight:500 }}>Servicio de interés</label>
              <select value={newCo.service} onChange={e=>setNewCo({...newCo,service:e.target.value})} style={{ width:'100%', padding:'9px 12px', border:'1px solid rgba(255,255,255,.2)', borderRadius:9, fontSize:13, background:'#1c2030', color:'#edf0f8', outline:'none' }}>
                {['Agente IA WhatsApp','Reservas automáticas','CRM + integraciones','Web premium','Atención 24/7','Email automation'].map(s=><option key={s}>{s}</option>)}
              </select>
            </div>
          </div>
          <div style={{ display:'flex', gap:8 }}>
            <Btn variant="primary" onClick={onSave}>✓ Guardar empresa</Btn>
...
// ── PIPELINE ───────────────────────────────────────────
function PipelinePage({ leads, allLeads, filter, setFilter, showNew, setShowNew, newLead, setNewLead, onSave }) {
  const presup = allLeads.filter(l=>l.status==='presupuesto_enviado').reduce((s,l)=>s+(l.amount||0),0)
  const ganado = allLeads.filter(l=>l.status==='pedido').reduce((s,l)=>s+(l.amount||0),0)
  const mant = allLeads.filter(l=>l.status==='mantenimiento').reduce((s,l)=>s+(l.monthly||0),0)
  const perdido = allLeads.filter(l=>l.status==='perdido').reduce((s,l)=>s+(l.amount||0),0)
  ...
  return (
    <div>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:5 }}>
        ...
        <Btn variant="primary" onClick={()=>setShowNew(!showNew)}>+ Nuevo lead</Btn>
      </div>
      ...
      <div style={{ display:'flex', gap:8, marginBottom:16, flexWrap:'wrap', alignItems:'center' }}>
        ...
        <select value={filter.user} onChange={e=>setFilter({...filter,user:e.target.value})} style={{ background:'#1c2030', border:'1px solid rgba(255,255,255,.2)', borderRadius:9, color:'#edf0f8', fontSize:12, padding:'7px 12px', cursor:'pointer', outline:'none' }}>
          ...
          {['AN','JO','IK','MI'].map(u=><option key={u} value={u}>{u}</option>)}
        </select>
      </div>

      {showNew && (
        <Card title="Nuevo lead" style={{ marginBottom:16 }}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            <div style={{ marginBottom:12 }}>
              <label style={{ fontSize:12, color:'#b8c0d8', marginBottom:5, display:'block', fontWeight:500 }}>Estado</label>
              <select value={newLead.status} onChange={e=>setNewLead({...newLead,status:e.target.value})} style={{ width:'100%', padding:'9px 12px', border:'1px solid rgba(255,255,255,.2)', borderRadius:9, fontSize:13, background:'#1c2030', color:'#edf0f8', outline:'none' }}>
                {PIPE_COLS.map(c=><option key={c.id} value={c.id}>{c.label}</option>)}
              </select>
            </div>
            <div style={{ marginBottom:12 }}>
              <label style={{ fontSize:12, color:'#b8c0d8', marginBottom:5, display:'block', fontWeight:500 }}>Comercial</label>
              <select value={newLead.assigned} onChange={e=>setNewLead({...newLead,assigned:e.target.value})} style={{ width:'100%', padding:'9px 12px', border:'1px solid rgba(255,255,255,.2)', borderRadius:9, fontSize:13, background:'#1c2030', color:'#edf0f8', outline:'none' }}>
                {['AN','JO','IK','MI'].map(u=><option key={u} value={u}>{u}</option>)}
              </select>
            </div>
          </div>
          <div style={{ display:'flex', gap:8 }}>
            <Btn variant="primary" onClick={onSave}>✓ Guardar lead</Btn>
...
function TeamPage({ leads }) {
  const team = [
    { ini:'A', name:'Andoni', email:'andoni@azeritek.es', role:'admin', color:'rgba(139,127,255,.25)', tc:'#c4beff', key:'AN' },
    { ini:'J', name:'Jon', email:'jon@azeritek.es', role:'admin', color:'rgba(59,158,255,.25)', tc:'#93c5fd', key:'JO' },
    { ini:'I', name:'Iker', email:'iker@azeritek.es', role:'comercial', color:'rgba(31,201,138,.2)', tc:'#6ee7b7', key:'IK' },
    { ini:'M', name:'Miren', email:'miren@azeritek.es', role:'comercial', color:'rgba(245,166,35,.2)', tc:'#fcd34d', key:'MI' },
  ]
  return (
    <Card title="Equipo comercial" action={<Btn variant="primary">+ Añadir miembro</Btn>}>
      {team.map((m,i)=>(
        <div key={i} style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 0', borderBottom: i<team.length-1?'1px solid rgba(255,255,255,.08)':'none' }}>
          ...
          <div style={{ flex:1 }}>
            <div style={{ fontSize:13, fontWeight:600, color:'#edf0f8' }}>{m.name}</div>
            <div style={{ fontSize:11, color:'#b8c0d8' }}>{m.email}</div>
          </div>
...
function ReportsPage({ leads, companies }) {
  ...
  return (
    <div>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12, marginBottom:20 }}>
        {[
          { val: leads.filter(l=>l.status==='pedido').length, label:'Pedidos confirmados', delta:'este trimestre', color:'rgba(31,201,138,.15)', tc:'#6ee7b7', icon:'✅' },
          { val: '€'+leads.filter(l=>l.status==='presupuesto_enviado').reduce((s,l)=>s+(l.amount||0),0).toLocaleString(), label:'Presupuestado activo', delta: leads.filter(l=>l.status==='presupuesto_enviado').length+' leads', color:'rgba(139,127,255,.15)', tc:'#c4beff', icon:'📋' },
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
        ...
        <Card title="Rendimiento por comercial">
          {[['Andoni','AN',65,'#8b7fff'],['Jon','JO',30,'#3b9eff'],['Iker','IK',20,'#1fc98a'],['Miren','MI',10,'#f5a623']].map(([name,key,pct,color],i)=>(
            <div key={i} style={{ display:'flex', alignItems:'center', gap:10, marginBottom:9 }}>
              <div style={{ fontSize:12, color:'#b8c0d8', width:150 }}>{name}</div>
              <div style={{ flex:1, height:7, background:'#1c2030', borderRadius:4, overflow:'hidden' }}><div style={{ width:`${pct}%`, height:'100%', background:color, borderRadius:4 }}></div></div>
              <div style={{ fontSize:12, color:'#edf0f8', width:30, textAlign:'right', fontWeight:600 }}>{leads.filter(l=>l.assigned===key).length}</div>
            </div>
          ))}
        </Card>
      </div>
...
function Btn({ children, onClick, variant, style }) {
  const configs = {
    primary: { bg:'#8b7fff', bd:'#8b7fff', co:'#ffffff', hbg:'#6a5fd4' },
    danger:  { bg:'rgba(239,68,68,.18)', bd:'rgba(239,68,68,.45)', co:'#fca5a5', hbg:'rgba(239,68,68,.3)' },
    default: { bg:'#1c2030', bd:'rgba(255,255,255,.28)', co:'#ffffff', hbg:'#2e3450' },
  }
